...
public class Jwt implements Constants {
	...
	public static String generate(String username) {
		...
		String jwtToken = Jwts.builder().setSubject(username).setId(UUID.randomUUID().toString())
				.setIssuedAt(Date.from(now)).setExpiration(Date.from(now.plus(JWT_EXPIRATION, UNIT))).signWith(hmacKey)
				.compact();
		return jwtToken;
	}
	...
	public static Jws<Claims> validate(String jwtToken) {
		...
		Jws<Claims> claims = Jwts.parserBuilder().setSigningKey(hmacKey).build().parseClaimsJws(jwtToken);
